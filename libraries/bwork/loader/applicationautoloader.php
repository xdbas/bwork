...
class Bwork_Loader_ApplicationAutoloader implements Bwork_Loader_Autoloader
{
    ...
    public static function autoload($className)
    {
        ...
        $autoload = array('model', 'vo', 'controller');

        if (count($autoload) > 0) {
            foreach ($autoload as $type) {
                if (substr($className, -strlen($type)) == $type) {
                    $fileName = substr($className, 0, strpos($className, $type)) . '.php';

                    return static::load($fileName, $type);
                    break;
                }
            }
        }
    ...
    public static function load($filename, $type)
    {
        ...
        if (self::fileExists($path . $filename)) {
            ...
            throw new Bwork_Loader_Exception(
                sprintf(
                    'File(%s) [%s] could not be found in any of the checked locations.',
                    $type,
                    $filename
                ));
        }
