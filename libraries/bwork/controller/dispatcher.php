...
class Bwork_Controller_Dispatcher
{
    ...
    public function dispatch(Bwork_Router_Router $router)
    {
        ...
        if (isset($module)) {
            ...
            $controllerPath = $config->get('module_path') . strtolower(
                    $module
                ) . DIRECTORY_SEPARATOR . $configModule['controller_path'];
        } else {
        ...
        if (Bwork_Loader_ApplicationAutoloader::fileExists(($filePath = $controllerPath . $filename)) === false) {
            if ($config->get('dev_env') === true
                ...
                $router->controller = $notfound_settings['controller'];
                $router->action = $notfound_settings['action'];
                $router->module = $notfound_settings['module'];
        ...
        if ($reflectionClass->isSubclassOf('Bwork_Controller_Action') === false) {
            throw new Bwork_Controller_Exception(sprintf(
                '[%s] have to be an instance of Bwork_Controller_Action',
                $controllerName
            ));
        }
